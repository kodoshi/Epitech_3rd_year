...
import defaultpost from "../images/eur_default.jpg";
import { withRouter } from "react-router";
import { deleteAllTrades, createtrade } from "./apiService.js";
...
class CreateTrade extends React.Component {
    ...
    /**
     * On the Component Mount we will save FormData on the tradeData variable
    ...
    deleteTradesOnClick() {
        ...
        deleteAllTrades(userId, tokenkey).then((data) => {
     ...
     */
    validate() {
        const { fileSize, action_eur } = this.state;
    ...
    handleChange = (name) => (event) => {
        ...
        //save the values on the tradeData
        this.tradeData.set(name, value);
    ...
    createservice() {
        if (this.validate()) {
            ...
            const userId = isAuthenticated().user._id;
            const tokenkey = isAuthenticated().token;
            //this.tradeData --> send the tradeData to the backend
            //this.tradeData will return a json response
            createtrade(userId, tokenkey, this.tradeData).then((data) => {
                if (data.error) this.setState({ error: data.error });
                //console.log("New Post:", data)
                else this.props.history.push(`/`);
            });
            ...
            <div className="edit">
                <div className="container cont">
                    <h2 className="mt-5 mb-5 text-center font1">
                        <FontAwesomeIcon icon={faPlusCircle} /> Exchange Info Service
          </h2>
                    <img
                        src={photoUrl}
                        alt={this.state.name}
                    ...
                    <button
                        ...
                        onClick={() => this.deleteTradesOnClick()}
                    >
                        {" "}
