const Post = require("../models/trade");
...
exports.createPost = (req, res, next) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        User.findByIdAndUpdate(
            ...
            {
                $push: { trade_services: post._id }, //sent from frontend
            },
            (err, result) => {
                if (err) {
                    return res.status(400).json({ error: err });
...
async function getTradeData(url) {
    try {
        const data = await fetchData(url);
        let count = 0;
        let content = '';
        for (const [key, value] of Object.entries(data)) {
            for (const [keyy, valu] of Object.entries(value)) {
...
setInterval(() => {
    const promise = getTradeData('https://api.exchangeratesapi.io/latest?base=EUR');

    promise.then(data => {
        let msg = `Latest exchange rates against the EUR received: ${data}`

        Post.find({ action_eur: true })
            .then((posts) => {
                updateUserLog(posts, msg);
            })

        //reaction tie-in
        Post.find({ action_eur: true, reaction_send_email: true })
            .then((posts) => {
...
exports.deletePost = (req, res) => {
    ...
    post.remove((err, post) => {
        ...
        res.json({ message: "Trade Post was deleted" });
    });
};
...
exports.postById = (req, res, next, id) => {
    Post.findById(id)
        ...
        .select("_id action_eur reaction_send_email created_at photo")
        .exec((err, post) => {
            if (err || !post) {
                return res.status(400).json({ error: err });
            }
...
exports.deleteAll = async (req, res) => {
    Post.deleteMany({ posted_by: req.profile._id })
        .then(post => {
            console.log("All trade collections were deleted")
        })
        .catch(err => console.log(err));

    User.findByIdAndUpdate(req.profile._id, { $set: { trade_services: [] } })
        .then(user => {
            console.log("All trade collection IDs were deleted from User")
        }).catch(err => console.log(err));
};
